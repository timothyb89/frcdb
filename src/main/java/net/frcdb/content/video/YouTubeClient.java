...
public class YouTubeClient {
	...
	/**
	 * Creates a new YouTubeVideo by fetching information on the provided
	 * video id from YouTube via a JSON request.
	...
	public YouTubeVideo getVideo(String id) throws IOException {
		...
		Iterator<JsonNode> thumbIter = mediaGroup.get("media$thumbnail")
				.getElements();
		while (thumbIter.hasNext()) {
			JsonNode thumbElement = thumbIter.next();
	...
	public static String getVideoId(String url) {
		System.out.println("Attempting to extract video id from: " + url);
		try {
			URI uri = new URI(url);
