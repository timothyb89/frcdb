 ...
 * @author tim
 */
public class TabTagHandler extends BodyTagSupport {
	...
	private void writeTagBodyContent(JspWriter out, BodyContent bodyContent) throws IOException {
		//String clazz = selected ? "jquery-tab-selected" : "jquery-tab";

		Tab tab = new Tab();
		tab.setId(id);
		tab.setTitle(title);
		tab.setSelected(selected);
		tab.setContent(bodyContent.getString());
		
		TabPaneTagHandler pane = (TabPaneTagHandler) 
				findAncestorWithClass(this, TabPaneTagHandler.class);
		
		if (pane == null) {
			System.err.println("[Error] No TabPane parent exists for Tab");
			return;
		}
	...
	private void handleBodyContentException(Exception ex) throws JspException {
		...
		throw new JspException("Error in TabTagHandler tag", ex);
	}
