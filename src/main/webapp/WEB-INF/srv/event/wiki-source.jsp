<%-- 
    ...
    Created on : Sep 25, 2010, 2:07:29 AM
    Author     : tim
--%>
...
<% } else if (revision == null) { %>
	The given revision (<%= rev %>) does not exist or is invalid.
<% } else { %>
<code style="display: block; border-bottom: 1px solid grey; padding-bottom: 1em"><%= revision.getContent().replace("\n", "<br>\n") %></code>
<% } %>
<%
Date d = revision.getDate();
String date = new SimpleDateFormat("EEEE, MMMM dd, yyyy").format(d);
String time = new SimpleDateFormat("HH:mm").format(d);
%>
Raw text of '${wikiPageName}', revision #${wikiPage.latestRevisionNumber + 1}
